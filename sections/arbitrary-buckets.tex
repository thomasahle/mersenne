...
\begin{algorithm}\label{alg:b-bit-arb-r}
   ...
   $j_x\gets h_x\andtt(2^{b-1}-1)$\hfill $\rhd\quad j_x$ gets $b-1$ least 
   significant bits of $h_x$\\
   $i_x\gets (rj_x)\rs (b-1)$\hfill $\rhd\quad i_x$ is most uniform
   in $[r]$\\
...
\begin{algorithm}\label{alg:Mersenne-arb-r}
   ...
   $j_x\gets h_x\andtt(2^{b-1}-1)$\hfill $\rhd\quad j_x$ gets $b-1$ least 
   significant bits of $h_x$\\
   $i_x\gets (rj_x)\rs b-1$\hfill $\rhd\quad i_x$ is quite uniform
   in $[r]$\\
   $a_x\gets h_x\rs (b-1)$\hfill $\rhd\quad a_x$ gets the most significant bit of $h_x$\\
