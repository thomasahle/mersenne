...
% \end{equation}
% Therefore we still have the same bound \eqref{eq:E-F2-p} as in Theorem \ref{thm:h-and-s-p}.
...
% which by \eqref{eq:ix=0} in Lemma \ref{lem:ix-r-dist} is bound by
% $(1+r/2^b)/r$. As a result, for the total contribution of these terms,
% we have to multiply the $4 F_2^2 n/(rp^2)$ from \eqref{eq:one-pair} by
...
% \eqref{eq:Var-b-bit-arb-r} with uniform $b$-bit hash values; namely that
...
% \eqref{eq:Var-Mersenne-arb-r}. The basic conclusion is that both with
...
% \eqref{eq:E-Mersenne-arb-r}. Relative to the correct value $F_2$, the
