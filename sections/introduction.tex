...
we can get away with only testing the possible off-by-one in \Cref{eq:Mersenne} once, rather than at every loop.
...
symmetric. Hence we can select any $\ell$ bits for the
...
The basic setup is as follows.  For keys in $[u]$ and integer values in $\Z$, we are given a stream of key/value $(x_0,\Delta_0),\ldots, (x_{n-1},\Delta_{n-1})\in [u]\times\Z$. The
