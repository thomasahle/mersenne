...
\begin{algorithm}\label{alg:division-generalized}
   \caption{Generalized Division by Mersenne Primes.
      For $m$ such that $x \le (2^n/c)^m$ calculates
      $v = \floor{\frac{x}{2^n-c}}$.
   }\label{euclid}
   \begin{algorithmic}[1]
...
In the Crandal case $x\le 2^n$ and $c\le 2^{\floor n/2}$, as described by the Encyclopedia of Cryptography and Security~\cite{van2014encyclopedia}, we can let $m=2$ and the algorithm runs in just two steps for a total of six operations:
   $$
   \left\lfloor\frac{x}{2^n-c}\right\rfloor
   = \left\lfloor{\frac{\left(\lfloor\frac{(x+1)c}{q}\rfloor+x+1\right)c}{q}\right\rfloor
