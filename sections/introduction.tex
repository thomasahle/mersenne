...
\begin{figure}
   ...
   \begin{tikzpicture}[darkstyle/.style={circle,draw,fill=gray!40,minimum size=20}]
      \newcommand*{\figb}{8}
      \draw[pattern=north west lines, pattern color=blue] (1,0) rectangle (\figb,1);
      \draw[pattern=north west lines, pattern color=green] (0,0) rectangle (1,1);
      \foreach \x in {0,...,\figb-1} {
         \draw (\x, 0) -- (\x, 1);
         \node [draw=none] at (.5+\x,.5) {
               %\pgfmathparse{\pgfmathresult*(random())}
               0
            };
      }

   \end{tikzpicture}
   \caption{The output bits of a random polynomial modulo $2^b-1$, can be considered perfectly uniform, except the first one.}
   \label{fig:bits}
\end{figure}
