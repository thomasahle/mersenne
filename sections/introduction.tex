...
\begin{figure}
   ...
   \begin{tikzpicture}[darkstyle/.style={circle,draw,fill=gray!40,minimum size=20}]
      ...
      % Most of the numbers
      \foreach \y in {0,...,2}
         \foreach \x in {0,...,\figb-3}
            \node [draw=none] at (.5+\x,.5+\y) {1};
      \foreach \x in {\figb-3,...,\figb-1}
         \node [draw=none] at (.5+\x,.5+3) {.};
      \foreach \y in {4,...,5}
         \foreach \x in {0,...,\figb-2}
   ...
   \end{tikzpicture}
   \caption{Each output bit of a random polynomial modulo $2^b-1$ has the same identical distribution, which is only very slightly biased towards 0.}
   \label{fig:bits}
\end{figure}
...
%, for $k>2$,
wanted to implement k-independent hashing efficiently using standard portable code.
...
The k-independent hashing with a polynomial uses $O(k)$ space and $O(k)$ time
to compute the hash value of a key. Siegel \cite{Siegel04} has proved that if we want k-independent hashing in time $t<k$, then we need to use space $u^{1/t}$.
...
The classic definition of $k$-independent hashing (or $k$-universal)
goes back to Carter and Wegman \cite{wegman81kwise}.
\begin{definition}
   A random hash function $h:U\to R$ is $k$-independent if for $k$
   distinct keys $x_0,\ldots,x_{k-1}\in U$, the values
   $h(x_0),\ldots,h(x_{k-1})$ are independent.
\end{definition}
The classic example of $k$-independent
...
normally used for fast computation of $k$-independent hash functions.
...
\footnote{We note that $k=2$, we do have the fast multiply-shift scheme of Dietzfelbinger~\cite{dietzfel96universal}, that directly gives 2-independent
...
We assume a $k$-independent hash function $h:[u]\to[p]$, e.g.,
...
Suppose we had a $k$-independent hash function into $b$-bit strings.
...
Let $h:U\to [2^b]$ be $k$-independent. By definition this
...
it uses two independent 4-independent hash functions
$i:[u]\to[r]$ and $s:[u]\to\{-1,1\}$, but 4-independent hash functions
...
$i:[u]\to[t]$ and $s:[u]\to\{-1,1\}$ using a single $4$-independent
...
$s(x)=1-2a(x)$. It is then easy to show that if $h$ is $k$-independent
...
\end{algorithm}
% \begin{lemma}\label{lem:b-bit-hashing} Suppose $h:[u]\to[2^b]$ is $k$-independent. Let
%    $i:[u]\to[2^\ell]$ and
%    $s:[u]\to\{-1,1\}$ be constructed from $h$ as described in Algorithm \ref{alg:h-and-s}. Then $h$ and $s$ are both $k$-independent. Moreover, for
...
that $h$ is $k$-independent into $[2^b]$, which in particular implies that
the hash values are uniform in $[2^b]$.


\subsubsection{Two-for-one hashing with  Mersenne primes}\label{sec:two-for-one}
Above we discussed how useful it would be with $k$-independent hashing
...
of a $k$-independent hash function $h:[u]\to[2^b-1]$. The hash values
...
$k$-independent hash function $h:[u]\to[2^b-1]$ from which we construct
...
\begin{theorem}\label{thm:h-and-s-p}
   ...
   Mersenne prime.
   Suppose with have a $k$-independent hash function $h:[u]\to[2^b-1]$, e.g.,
...
Suppose we have a $2$-independent hash function $h:U\to Q$.
...
of of having $k$-independent hashing into $[2^b]$.\todo{Add comment about it being efficient for Mersenne aswell using our result!}
