...
We now consider what would happen with our bit-selection if
we had used an  arbitrary prime $p=2^b-c$, $c\in [1,2^{b-1})$.
...
the $\ell$ least respectivily most significant bits. If
...
we get $\Pr[\mu(h(x))=0]=2^{b-\ell}/p$. As an extreme case, when $\ell=1$
and $p=2^{b-1}+1$, we get $\Pr[\mu(h(x))=0]=1-1/p$ and $\Pr[\mu(h(x))=1]=1/p$.
...
\ref{sec:power-of-two}. If we instead had used a the $b$-bit prime
$p=2^{b-1}+1$, then the signbit $a_x$ would be extremely biased with
